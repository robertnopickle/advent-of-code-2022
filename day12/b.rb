...
class Node
  ...
  def can_decend_to_neighbor?(neighbor)
...
until $unvisited.empty?
  ...
  current_neighbors.each do |neighbor|
    next unless current_node.can_decend_to_neighbor?(neighbor)
...
possible_ends = $visited.select { |x| x.value == 0 && x.count_of_previous_nodes > 0 }.map(&:count_of_previous_nodes)
puts "path length: #{possible_ends.min}"
